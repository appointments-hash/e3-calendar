...
export const handler = async (event) => {
  try {
    ...
    const id = body.id;
    if (!calendarId || !id) return jsonResponse(400, { ok:false, message:"calendarId and id are required" });
    ...
    if (startISO) updated.start = { dateTime: startISO };
    if (endISO) updated.end = { dateTime: endISO };
